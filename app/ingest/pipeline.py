...
class Ingest:
    ...
    async def assemble(self, topic: dict, anchor: Message, items: list[Item]) -> list[dict]:
        ...
        # batch items strictly in Telegram order: message ids are generated in the order the user sent them
        parts.extend(sorted(items, key=lambda i: i.message_id or 0))
        blocks: list[dict] = []
        text_acc: list[str] = []
