...
def help(user_id: int, chat_id: int, thread_id: int | None) -> str:
    return (
        ...
        "/plan — режим плана (только чтение, план карточкой); /auto — Claude Code решает сам\n"
...
REWIND_LIST = "Откатить файлы к состоянию на момент сообщения (правки после него исчезнут; сессия и чат останутся):"
REWIND_EMPTY = "Чекпойнтов ещё нет: они появляются у ходов, начатых с включённым FILE_CHECKPOINTING."
REWIND_CONFIRM = "Откатить файлы к моменту «{prompt}»? Правки Claude Code после этого сообщения исчезнут."
REWIND_DONE = "⏪ {text}"
REWIND_FAILED = "⚠️ Откат не удался: {reason}"
TOAST_REWOUND = "Файлы откачены"
