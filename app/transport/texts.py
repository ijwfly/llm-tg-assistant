...
HELP = (
    ...
    "/sessions — все сессии Claude Code на машине; кнопки продолжают здесь или открывают тему под папку\n"
    "/resume <id|имя> — подключить сессию к этой теме\n"
    "/branch [имя] — новая тема с копией контекста; /project <алиас|путь> — тема под папку; /project new <имя> — новая папка\n"
    "/rename <имя> — переименовать тему и сессию; /delete — удалить тему (темы, созданные ботом, ты сам удалить не можешь)\n"
    "/perm [режим] — права темы; /perm forget — забыть правила «Всегда»\n"
    "/model [имя|default], /effort [low…max|default] — модель и усилие темы (есть и на карточке)\n"
    "/soul [путь|off|default] — персона (файл в системный промпт); /voice on|off — голосовые ответы\n"
    "/usage — расход за месяц по темам и моделям\n"
    "/topics — список тем\n"
