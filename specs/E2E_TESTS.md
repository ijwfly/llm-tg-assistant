...
Status: все фазы — инфраструктура, команды, outbox, ходы, стриминг, кнопки, входной конвейер, разрешения/вопросы/планы, сессии и темы, настройки/персона/голос/расход, send_file/вывод инструментов/rewind; обновляется каждой фазой.
...
| `e2e/test_cards.py` | `/status` → карточка → callbacks → `actions` | кнопки карточки; `🛑` при идущем ходе; `🆕` перерисовывает карточку; `⏸`/`🔄`; `✖` удаляет; устаревшая кнопка → тост; `🔁` на вердикте; `🔓` → `acceptEdits` и новый argv; `▶️ Продолжить`; `/perm`; меню = зарегистрированные команды минус алиасы и скрытая `/soul` |
...
| `e2e/test_sessions.py` | `/sessions`, `/resume` → индекс транскриптов → тема | карта всей машины: своя папка первой, пометки «эта тема / эта тема, раньше», кнопки `Продолжить здесь`/`Новая тема`, подвал про сессии вне `WORK_ROOT`; страницы по `SESSIONS_PAGE_SIZE` — «Дальше»/«Назад» редактируют карточку на месте, заголовок `стр. N/M`; карточка в markdown (папки жирным, экранирование заголовков), подписи кнопок — имя папки (время при повторе), обрезка до 48; `Новая тема` → тема с именем папки сессии и `--resume`; тема без явного имени переименовывается при переезде в другую папку, после `/rename` — нет; прошлая сессия после `/new`; пусто; `/resume` по префиксу → id/resumable и `--resume` в argv; переезд в cwd сессии; недоступная cwd → остаёмся с предупреждением; без аргумента/неизвестная/неоднозначная/по имени/полный id; кнопка `🔗`; `/status` с заголовком |
...
| `e2e/test_topics_delete.py` | кнопка «Удалить тему» / `/delete` → подтверждение → `DeleteForumTopic` (прямой) | подтверждение и удаление (строка, ходы, runtime), «Отмена» перерисовывает карточку, отказ Telegram оставляет тему, в самом чате — отказ |
| `e2e/test_settings.py` | карточка темы → `cyc`/`tgl`/`page` → `actions` → `prefs` → argv / рендер | циклы прав/модели/усилия с перерисовкой и argv; обёртка цикла и `bypass` по `ALLOW_BYPASS`; страница «Ещё»: тогглы темы и пользователя, статистика хода; «Размышления: выкл» убирает 🧠 из draft; «Голосом» без `TTS_CMD`; `/model`, `/effort`; `/plan` и `/auto` меняют режим и argv следующего спавна; преамбула + персона в одном файле, `/soul off`, плохой путь; `SOUL_PATH` по умолчанию |
| `e2e/test_voice.py` | финал хода → `voice_for_turn` → `TTS_CMD` → `SendVoice` | проза после текста без кода; off и ход без текста; провал TTS молча; `/voice on` без `TTS_CMD` |
| `e2e/test_usage.py` | `turns.model` из init, `/usage`, `rate_limit_event` | карточка по темам/моделям/итог, «Лимиты» в `/status`; пусто |
| `e2e/test_send_file.py` | `claude` → MCP `send_file` → сокет → `PromptService.send_file` → outbox | фото + документ с подписью и ответом модели, порядок; вне корня / нет файла / пустой путь → `Error`; флаг off → инструмента нет в `tools/list`; prompt-запрос на `send_file` разрешён без карточки |
| `e2e/test_verbose.py` | `tool_result` → `<details>`, подагенты, 👾 | вывод инструментов с именем/деталью, ⚠️ и обрезка, порядок с текстом; off — тишина; `FORWARD_SUBAGENT_TEXT` → argv и `<details>`; 👾 на якоре при ошибке хода и падении, нет при успехе и при выключенных реакциях |
| `e2e/test_rewind.py` | echo user-сообщения → `turns.checkpoint_uuid` → «Откатить файлы» → standalone `--rewind-files` | чекпойнты записаны, кнопка при `FILE_CHECKPOINTING`, список и подтверждение, одноразовый вызов с нужным uuid и env, процесс темы остановлен; без флага кнопки нет, env нет |
| `e2e/test_rename.py` | `/rename`, implicit-темы | `EditForumTopic` + БД + тихий ход `/rename`; без имени; implicit-тема названа по своей папке сразу при первом сообщении, без хода `/rename`, один раз; явное имя не трогается |
...
| `unit/test_tts_text.py` | `render/tts.py` | код/таблицы/ссылки/разметка выброшены, обрезка по предложению, пусто для одного кода |
