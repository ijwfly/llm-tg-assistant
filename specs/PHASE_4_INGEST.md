...
| Вопрос | Решение |
|---|---|
| Батчер | `Batcher` в памяти: ключ — тема; скользящее окно `BATCH_WINDOW_MS` (300); флаш → сортировка по `message_id`; якорь — первое сообщение (reply-цитата). В ход элементы ложатся строго по `message_id` (порядок отправки). Команды не батчатся. |
...
- Живая проверка: Telegram отправляет комментарий к форвардам раньше самих форвардов (id
  комментария меньше). Решение пользователя: внутри батча порядок строго по `message_id`, без
  группировки по типам — id отражают порядок отправки. Staging по-прежнему идёт перед батчем.
