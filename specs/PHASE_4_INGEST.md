...
| Вопрос | Решение |
|---|---|
| Батчер | `Batcher` в памяти: ключ — тема; скользящее окно `BATCH_WINDOW_MS` (300); флаш → сортировка по `message_id`; якорь — первое сообщение (reply-цитата, реакции). Команды не батчатся. |
