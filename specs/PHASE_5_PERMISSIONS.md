...
- **2026-09-04**: `DEFAULT_PERMISSION_MODE` по умолчанию стал `auto` (решение пользователя: меньше карточек).
  Только для новых тем; prompt tool в `auto` по-прежнему подключается, факт «спорное доходит до карточки»
  остаётся ненаблюдённым (п. 7 чеклиста).
